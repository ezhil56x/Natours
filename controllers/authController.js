...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    expires: new Date(
      Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 10 * 60 * 1000
    ),
    httpOnly: true
  };
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!freshUser) {
    return next(
      new AppError('The user belonging to this token on longer exist', 401)
    );
  }

  // 4) Check if user changed password after the toek token was issued
